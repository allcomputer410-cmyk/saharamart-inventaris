...
def _check_autostart() -> bool:
    ...
    reg_name = "iPOS_SyncAgent_Arafamart"
    try:
...
def main():
    ...
    logging.info("=" * 60)
    logging.info(f"Sync Agent ARAFAMART (ARM01) dimulai — {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}")
    logging.info("=" * 60)
